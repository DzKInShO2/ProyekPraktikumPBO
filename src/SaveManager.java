...
public class SaveManager {
    ...
    SaveManager.saveFilePath = saveFilePath; // FIX
...
public static int getGridX() {
    try (
        ...
        if (rs.next()) return rs.getInt("grid_x");
    } catch (SQLException e) {}
    return 0;
}
...
public static int getGridY() {
    try (
        ...
        var rs = statement.executeQuery("SELECT grid_y FROM progress");
        if (rs.next()) return rs.getInt("grid_y");
    } catch (SQLException e) {}
    return 0;
}
